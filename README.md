...
모바일 브라우저에서 카트 단위 박스 라벨 촬영 순서를 관리하기 위한 정적 웹앱 초안입니다. 이 저장소의 MVP는 OCR, 서버, DB, 자동 업로드 없이 촬영 진행 상태, 카메라 미리보기, JPEG 임시 캡처, 내보내기용 manifest 구조를 확인하는 데 집중합니다.
...
- 현재 브라우저 세션 동안 박스별 사진 미리보기와 이미지 메타데이터를 보여줍니다.
- `manifest.json` / `manifest.csv` 미리보기에 파일 경로와 이미지 메타데이터를 포함합니다.
- Blob과 object URL은 메모리에만 두고, 가벼운 세션 상태만 `localStorage`에 저장합니다.
...
현재 사진은 런타임 메모리에만 보관됩니다. 새로고침하면 `localStorage`의 촬영 완료 상태는 복원될 수 있지만, IndexedDB가 구현되기 전까지 실제 사진 미리보기는 사라질 수 있습니다.
...
GitHub Pages는 HTTPS를 제공하므로 이후 실제 기기 테스트와 배포에 적합합니다. 현재 단계는 카메라 프레임을 JPEG Blob으로 캡처하지만 사진 파일을 영구 저장하거나 ZIP으로 내보내지는 않습니다.
