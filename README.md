...
모바일 브라우저에서 카트 단위 박스 라벨 촬영 순서를 관리하기 위한 정적 웹앱 초안입니다. 이 저장소의 MVP는 OCR, 서버, DB, 자동 업로드 없이 촬영 진행 상태, 카메라 미리보기, JPEG 캡처, IndexedDB 사진 저장, 내보내기용 manifest 구조를 확인하는 데 집중합니다.
...
GitHub Pages는 HTTPS를 제공하므로 이후 실제 기기 테스트와 배포에 적합합니다. 현재 단계는 카메라 프레임을 JPEG Blob으로 캡처하고 IndexedDB에 저장하지만 ZIP으로 내보내지는 않습니다.
