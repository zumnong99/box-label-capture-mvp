...
- 정지사진 API가 없는 iPhone Safari에서는 현재 video 프레임을 canvas로 캡처하는 방식으로 자동 폴백합니다.
...
- 카메라 바로 아래에서 `사진 + manifest ZIP 만들기`를 먼저 실행하고 `완성된 ZIP 저장`으로 내려받습니다.
- Chromium Android는 보안상 ZIP을 Web Share 허용 형식에 포함하지 않으므로, 카카오톡·Instagram 등의 시스템 공유창은 JPEG 사진 공유용으로 분리합니다.
